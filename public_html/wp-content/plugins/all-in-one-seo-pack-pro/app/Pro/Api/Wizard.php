...
class Wizard extends CommonApi\Wizard {
	...
	public static function saveWizard( $request ) {
		...
		if (
			...
			$cantInstall = [];
			if ( in_array( 'local-seo', $features, true ) ) {
				$addon = aioseo()->addons->getAddon( 'aioseo-local-business', true );
			...
			if ( in_array( 'image-seo', $features, true ) ) {
				$addon = aioseo()->addons->getAddon( 'aioseo-image-seo', true );
				if ( ! $addon->isActive && ! $addon->requiresUpgrade ) {
					if ( $addon->installed || $addon->canInstall ) {
						aioseo()->addons->installAddon( $addon->basename );
					} else {
			...
			if ( in_array( 'video-sitemap', $features, true ) ) {
				$addon = aioseo()->addons->getAddon( 'aioseo-video-sitemap', true );
				if ( ! $addon->isActive && ! $addon->requiresUpgrade ) {
					if ( $addon->installed || $addon->canInstall ) {
						aioseo()->addons->installAddon( $addon->basename );
					} else {
			...
			if ( in_array( 'news-sitemap', $features, true ) ) {
				$addon = aioseo()->addons->getAddon( 'aioseo-news-sitemap', true );
				if ( ! $addon->isActive && ! $addon->requiresUpgrade ) {
					if ( $addon->installed || $addon->canInstall ) {
						aioseo()->addons->installAddon( $addon->basename );
					} else {
