...
class TermMeta {
	...
	public function scheduleImport() {
		try {
			...
			if ( ! get_transient( 'aioseo_import_term_meta_rank_math' ) ) {
				set_transient( 'aioseo_import_term_meta_rank_math', time(), WEEK_IN_SECONDS );
			}
	...
	public function importTermMeta() {
		...
		$timeStarted      = gmdate( 'Y-m-d H:i:s', get_transient( 'aioseo_import_term_meta_rank_math' ) );
		...
		if ( ! $terms || ! count( $terms ) ) {
			delete_transient( 'aioseo_import_term_meta_rank_math' );
			return;
		}
